# -*- coding: utf-8 -*-
"""
Created on Thu Aug 26 10:33:15 2021
...
ul.set_config(InfoType.BOARDINFO, board_num, RL_temp_chanAC, BoardInfo.CHANTCTYPE, TcType.K)
ul.set_config(InfoType.BOARDINFO, board_num, ambient_chan, BoardInfo.CHANTCTYPE, TcType.K)

def meas_RL_temp():
    return ul.t_in(board_num, RL_temp_chanAC, TempScale.CELSIUS)
def meas_ambient():
...
qtCreatorFile = "Qi3.ui"
Ui_MainWindow, QtBaseClass = uic.loadUiType(qtCreatorFile)

class MyApp(QMainWindow, Ui_MainWindow):
    
    RL_loss = pyqtSignal(float)
    ...
    def __init__(self,):
        ...
        self.RL_loss.connect(self.RL_LossLCD.display)
    ...
    def update_vals(self):
        ...
        self.RL_power.emit(round(calc_power(RLrms),3))
        tcoil = meas_RL_temp()
        tamb = meas_ambient()
        self.RL_loss.emit(round(calc_loss(tcoil, tamb),3))
        print(f'tcoil = {tcoil:.3f}C')
        print(f'tamb = {tamb:.3f}C')
